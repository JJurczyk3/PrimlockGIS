...
def make_terrain_tin() -> TinModel:
    return TinModel(
        vertices=[
            TinVertex(id=0, x=0.0, y=0.0, z=0.0),
            TinVertex(id=1, x=100.0, y=0.0, z=10.0),
            TinVertex(id=2, x=0.0, y=100.0, z=20.0),
        ],
        triangles=[
...
def test_draw_tin_terrain_paints_background_cells_inside_triangle():
    renderer = make_renderer(
        TerminalCapabilities(
            supports_color=True,
            supports_truecolor=True,
    ...
    assert renderer.canvas.cells[3][2].background is not None
    assert renderer.canvas.cells[2][8].background is None


def test_terrain_opacity_blends_with_theme_background():
    renderer = make_renderer()
    style = TerrainStyle(
        low_color="#FFFFFF",
        low_mid_color="#FFFFFF",
        high_mid_color="#FFFFFF",
        high_color="#FFFFFF",
        background_color="#000000",
        opacity=0.5,
    )

    color = renderer._terrain_color(0.0, 0.0, 1.0, style)
